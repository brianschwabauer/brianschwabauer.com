<script lang="ts">
	...
	function update() {
		...
		const gap = 10;
		left = Math.max(8, Math.min(window.innerWidth - menuRect.width - 8, centerX - menuRect.width / 2)) + scrollX;
	...
	function toggleH3() { editor?.chain().focus().toggleHeading({ level: 4 }).run(); }
...
<div
	...
	{:else}
		<button type="button" class="bm-btn" class:active={editor?.isActive('bold')} onclick={() => run(toggleBold)} title="Bold (⌘B)"><strong>B</strong></button>
		<button type="button" class="bm-btn" class:active={editor?.isActive('italic')} onclick={() => run(toggleItalic)} title="Italic (⌘I)"><em>I</em></button>
		<button type="button" class="bm-btn" class:active={editor?.isActive('strike')} onclick={() => run(toggleStrike)} title="Strikethrough"><s>S</s></button>
		<button type="button" class="bm-btn" class:active={editor?.isActive('code')} onclick={() => run(toggleCode)} title="Inline code"><code>&lt;/&gt;</code></button>
		<span class="bm-sep"></span>
		<button type="button" class="bm-btn small" class:active={editor?.isActive('heading', { level: 2 })} onclick={() => run(toggleH1)} title="Heading 1">H1</button>
		<button type="button" class="bm-btn small" class:active={editor?.isActive('heading', { level: 3 })} onclick={() => run(toggleH2)} title="Heading 2">H2</button>
		<button type="button" class="bm-btn small" class:active={editor?.isActive('heading', { level: 4 })} onclick={() => run(toggleH3)} title="Heading 3">H3</button>
		<span class="bm-sep"></span>
		<button type="button" class="bm-btn" class:active={editor?.isActive('blockquote')} onclick={() => run(toggleQuote)} title="Quote">
		...
		<button type="button" class="bm-btn" class:active={editor?.isActive('link')} onclick={openLink} title="Link">
			<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14">
...
<style>
	...
	.bm-btn:hover { background: var(--color-bg-secondary); }
	.bm-btn.active { background: var(--color-accent); color: white; }
	.bm-btn.small { font-size: var(--text-xs); }
	.bm-btn code { font-family: var(--font-mono); font-size: 0.7rem; }
	.bm-btn.ai { color: var(--color-accent); }
	.bm-btn.ai:hover { background: var(--color-accent-light); }
	...
	.link-input:focus { border-color: var(--color-accent); }
</style>
